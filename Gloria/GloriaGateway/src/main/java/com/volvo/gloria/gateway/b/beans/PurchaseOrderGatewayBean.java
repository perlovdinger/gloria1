...
/**
 * This is the service activator for the Purchase Order storage Interface, which receives Purchase Orders.
 */
@ContainerManaged
public class PurchaseOrderGatewayBean implements MessageListener {

    private static final Logger LOGGER = LoggerFactory.getLogger(PurchaseOrderGatewayBean.class);
    
    private static final String APPLICATION = "GPS";
    private static final String INTEGRATION = "PurchaseOrderGateway";
    ...
    public void onMessage(Message msg) {
        if (msg instanceof TextMessage) {
            TextMessage receivedPurchaseOrderMessage = (TextMessage) msg;
            String jmsMessageID = "";
            String textMessage = "";
            try {
                jmsMessageID = msg.getJMSMessageID();
                textMessage = receivedPurchaseOrderMessage.getText();
                procurementServices.createOrUpdatePurchaseOrder(purchaseOrderTransformer.transformStoredPurchaseOrder(textMessage));
            } catch (Exception e) {
                LOGGER.error("An error occured while receiving a Purchase Order message [ID:" + jmsMessageID + "]: " + e.getMessage());
                GloriaExceptionLogger.log(e, PurchaseOrderGatewayBean.class);
            } finally {
                LOGGER.info("xml=" + textMessage);
                try {
                    ...
                    LOGGER.error("An error occured while saving a incoming Purchase Order message [ID:" + jmsMessageID + "]: " + e.getMessage(), e);
                }
            }
        } else {
